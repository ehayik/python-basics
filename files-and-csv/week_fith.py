...
"""
medals = {'Japan':41, 'Russia':56, 'South Korea':21, 'United States':121, 'Germany':42, 'China':70}
items = sorted(medals.items(), reverse=True, key= lambda kv: kv[1])
items = list(map(lambda kv: kv[0], items))
top_three = items[:3]


"""
We have provided the dictionary groceries. 
You should return a list of its keys, but they should be sorted by their values, 
...
items = sorted(groceries.items(), reverse=True, key= lambda kv: kv[1])
most_needed = [kv[0] for kv in items]


"""
Create a function called last_four that takes in an ID number and 
returns the last four digits. For example, the number 17573005 should return 3005. 
