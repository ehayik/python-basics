...
for every month (in inches) with every month separated by a comma. Write code to compute the 
number of months that have more than 3 inches of rainfall. Store the result in the variable 
num_rainy_months. In other words, count the number of items with values > 3.0.
Hard-coded answers will receive no credit.
"""
rainfall_mi = "1.65, 1.46, 2.05, 3.03, 3.35, 3.46, 2.83, 3.23, 3.5, 2.52, 2.8, 1.85"
val = [x for x in rainfall_mi.split(", ") if float(x) > 3.0]
num_rainy_months = len(val)
...
same_letter_count = len([x for x in sentence.split() if x[0] == x[-1]])
...
for val in sentence.split():
    if "a" in val or "e" in val:
        num_a_or_e += 1


"""
Write code that will count the number of vowels in the 
sentence s and assign the result to the variable num_vowels. For this problem, vowels are only 'a', 'e', 'i', 'o', and 'u'. 
